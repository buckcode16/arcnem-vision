...
エージェントグラフはコードではなくデータです。テンプレートがノード、エッジ、ツールを持つ再利用可能なワークフローを定義し、ダッシュボードではそのテンプレートを名前、ノードの役割、ツール名で探しながら新しい組織用ワークフローを起動できます。起動後のグラフは独立したコピーとして編集でき、元テンプレートの版情報も追えます。ノードタイプは4種類です。
...
すべての実行が`agent_graph_runs`と`agent_graph_run_steps`でステップごとに記録されます。OCRの生テキストと信頼度は`document_ocr_results`にも保存されるため、ダッシュボードから直接確認できます。Docsタブではこの同じデータを使って意味検索とコレクションチャットを並べて扱え、どの資料を根拠にした回答かも出典カードで追えます。

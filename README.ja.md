...
- **またはホスト型S3互換バケット** — AWS S3 / Cloudflare R2 / Railway Object Storage / Backblaze B2 など
...
`tilt up` はローカルスタック全体を起動し、TiltのダッシュボードUI（通常 `http://localhost:10350`）で以下を行えます。

- 各サービスのリアルタイムログ確認
- DBシードなど手動リソースの実行
- DBイントロスペクトなど補助フローの実行
